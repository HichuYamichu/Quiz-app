...
module.exports = {
	async createCollection(collName, questions) {
		...
		await db.mainDB().collection(collName).insertMany(questions);
	},
	...
	async fetchCollections() {
		const collections = await db.mainDB().listCollections().toArray();
	...
	async checkAnsweres(recivedAnswers, user) {
		const coll = await db.mainDB().collection(user.quiz).find().toArray();
		...
		await db.configDB().collection('scores').insertOne({ username: user.userName, quiz: user.quiz, score: points, max: collectionLength });
