        ...
        <v-card dark v-for="(question, index) in questions" :key="index" elevation-15>
         <v-layout row wrap>
           ...
           <v-flex xs6 offset-xs1 v-for="(answer, index2) in question.answers" :key="index2">
             <v-text-field :label="`Answer ${index + 1}`" outline v-model="answer.text">

             </v-text-field>
             <v-checkbox v-model="questions[index].correct[index2]" label="Correct" color="white"></v-checkbox>
...
export default {
  data() {
    return {
      collectionName: '',
      questions: [{ text: '', answers: [{ text: '', value: false}], correct: [] }]
    }
  },
  methods: {
    addQuestion: function() {
      this.questions.push({ text: '', answers: [{ text: '', value: false}], correct: [] })
    },
