...
export default {
  ...
  methods: {
    ...
    update: function() {
      this.$axios.$post('http://localhost:3000/api/update-collection', {name: this.collectionName, questions: this.questions})
    },
		find: async function() {
			const res = await this.$axios.$post('http://localhost:3000/api/fetch-collection', { name: this.collectionName })
